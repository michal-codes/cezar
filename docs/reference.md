...
| `CEZ_TRUSTED_HOSTS=a.ts.net:8445,b.ts.net` | Admit these exact `Host` authorities in **local** mode, in addition to loopback (the DNS-rebinding guard stays on: an unlisted Host is refused as before, and the `Origin`-vs-`Host` write guard is unchanged). Read per request. Use it to reach a cockpit over a private, device-authenticating network without switching to hosted mode — a trusted authority also reaches the local-only surfaces (agent-config editing, home-wide `fs/browse`, the launch key), so keep it private and never expose it publicly. Read per request; no restart needed. |
| `CEZ_PUBLIC_URL=https://my-node.ts.net:8445/` | The address the terminal banner prints and renders as a scannable QR code (that is what makes the phone path one step). Absolute URL only — the launch key is never appended. In local mode the authority should also be in `CEZ_TRUSTED_HOSTS`, or the banner warns that a scan would hit the guard. |
| `CEZ_NO_QR=1` | Silence the banner's QR (and its URL line) even when `CEZ_PUBLIC_URL` is set. Any CI environment silences it too; a loopback-only cockpit never prints one. |
